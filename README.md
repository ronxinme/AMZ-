...
**结论**：想免费拿到*精确*的竞品库存数字，目前没有可靠办法。能免费拿到的、而且更有决策价值的是「**断货监控**」——竞品断货就是你的机会窗口，这比一个静态库存数字有用得多。为此网页顶部做了专门的预警面板，见下一节。
...
- 具体库存数量：见上一节。库存列是状态口径：`仅剩 N 件`（亚马逊原话）/ `有货，数量未显示` / `缺货` / `无主报价` / `未知`
